        ...
        <div class="row">
                ...
                <div class="row">
                    <div v-if="searchCategory.length == 0 || searchCategory.length > 0" class="col-lg-3 mb-2 col-md-4 h-100 cc-show" v-for="(product,index) in currentPageProducts" :key="index">
                        <div class="card rounded box-product " @click="addToCart(product)">
                            <img :src="arrayimg[index]" alt="" class="card-img-top" width="60px" height="90px">
            ...
            <div class="col-lg-5 col-12 col-md-5 main-cart" style="border: 1px solid;">
                <form action="" method="post" @submit="findItem" class="d-flex justify-content-between align-items-center border-bottom p-1">
                    <input type="text" v-model="searchValue" class="form-control " placeholder="ระบุรหัสสินค้า">
                        ...
                        <tbody>
                            <tr v-for="(item,index) in cartProducts" :key="index">
                                ...
                                <td class="show-center" width="20%">
                                    <button class="bt-qty" @click="minusProduct(item)">-</button>
                                    <span class="text-primary">{{ item.product_qty }}</span>
                                    <button class="bt-qty" @click="plusProduct(item)">+</button>
                                </td>
                                <td class="show-right">{{ new Intl.NumberFormat().format(item.product_price * item.product_qty) }}</td>
                                <td class="show-center"><button class="btn btn-danger btn-sm" @click="removeProduct(item)"><i class="mdi mdi-close"></i></button></td>
                            </tr>
                        </tbody>
                    </table>
                </div>
...
<!-- Modal -->
<div class="modal fade" id="ModalSalepic">
