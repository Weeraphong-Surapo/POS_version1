<?php 
  ...
  if($res->action == "SaleProduct"){
    ...
    $sale_product = $conn->query("INSERT INTO `tb_sale`(`sale_code`, `customer_id`, `employee_id` , `count_cart`, `get_money`, `change_money`, `product_total_price` ,`by_date`, `by_month`) 
                                  VALUES ('$code','$res->customer_id','$_SESSION[employee_id]','$res->countcart','$res->getMo','$res->changeMo','$res->totalProduct' ,'$date','$months')");
    $print_id = $conn->insert_id;
    foreach($res->Cart as $k => $v){
        $price_toal_product = $v->product_price * $v->product_qty;
      $conn->query("INSERT INTO `tb_order`(`product_id`, `sale_id`, `product_qty`,`product_price`, `product_total_price`, `day`) 
                    VALUES ('$v->product_id','$print_id','$v->product_qty','$v->product_price','$price_toal_product', '$day')");
    }
