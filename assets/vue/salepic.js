var app = new Vue({
    ...
    methods: {
      ...
      saleOrder() {
        if (app.currentValue < app.totalPriceProduct) {
          ...
          app.changeMoneys = app.currentValue - app.totalPriceProduct;
      ...
      getAllProduct() {
        axios
          ...
          .then((res) => {
            ...
            for (let i = 0; i < this.AllProdcut.length; i++) {
              this.arrayimg.push('assets/' + this.AllProdcut[i].product_img);
          }
          });
      },
      getAllCategory() {
    ...
    computed: {
      ...
      filteredProducts() {
        ...
        return this.AllProdcut.filter((product) =>
          searchRegex.test(product.category_id)
        );
      },
      totalPages() {
